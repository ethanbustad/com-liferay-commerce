...
<div class="container-fluid-1280">
	...
	<liferay-util:buffer
		var="removeCommerceCatalogUserIcon"
	>
		<liferay-ui:icon
			icon="times"
			markupView="lexicon"
	...
	<aui:form action="<%= editCommerceCatalogUsersActionURL %>" cssClass="container-fluid-1280" method="post" name="fm">
		...
		>
			...
			>
				...
				<liferay-ui:search-container-column-text>
					<a class="float-right modify-link" data-rowId="<%= user.getUserId() %>" href="javascript:;"><%= removeCommerceCatalogUserIcon %></a>
				</liferay-ui:search-container-column-text>
			</liferay-ui:search-container-row>
...
String componentId = StringBundler.concat("userRolesModal", catalogUser.getUserId());
%>
			...
			function(userRolesModal) {
				userRolesModal.on(
					...
					function(selectedRoles) {
						var <portlet:namespace />selectedRoleIds = [];

						A.Array.each(
							selectedRoles,
							function(role) {
								<portlet:namespace />selectedRoleIds.push(role.id);
							}
						);
						...
						form.fm('selectedRoleIds').val(<portlet:namespace />selectedRoleIds.join(','));
			...
			var itemSelectorDialog = new A.LiferayItemSelectorDialog(
				{
					...
					on: {
						selectedItemChange: function(event) {
							var <portlet:namespace />addCommerceCatalogUserIds = [];
							...
							if (selectedItems) {
								A.Array.each(
									selectedItems,
									function(item) {
										<portlet:namespace />addCommerceCatalogUserIds.push(item.id);
									}
								);

								var form = AUI.$(document.<portlet:namespace />fm);

								form.fm('commerceCatalogUserIds').val(<portlet:namespace />addCommerceCatalogUserIds.join(','));
